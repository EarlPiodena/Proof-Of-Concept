...
# incomes = {doc.to_dict() for doc in incomes_ref.stream()}
# expenses = [isint(list(doc.to_dict().values())) for doc in expenses_ref.stream()][0]
...
def app():
    ...
    if selected == "Data Entry":
        ...
        with st.form("entry_form", clear_on_submit=True):
            ...
            #     comment = st.text_area("Write a comment", placeholder="Enter a comment here: ")
...
if __name__ == '__main__':
    ...
    initialize(creds_path=creds_path)
    db = firestore.client()
...
# """
# expenses_ref = db.collection(u'expenses')
# income_ref = db.collection(u'expenses')

# user_income = {'Name':'Earl', 'Salary':0, ... }
# user_expenses = {'Name':'Earl', 'Rent':0, ... }

# wrapper = [user_income, ]

# expenses_doc_ref = expenses_ref.document(user_income['Name'])
# income_doc_ref = income_ref.document('nigga')

# income_doc_ref.set(user_income)
